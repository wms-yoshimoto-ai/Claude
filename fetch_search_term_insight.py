...
【出力列】
  ...
  表示回数, クリック数, 通貨コード, 費用,
  コンバージョン, すべてのコンバージョン, コンバージョン値,
  クリック率, コンバージョン率, 平均クリック単価, コンバージョン単価
...
def row_to_csv_format(r: dict, daily: bool = False) -> dict:
    ...
    cpa = round(cost_yen / conv) if conv > 0 else None
    ...
    row = {
        ...
        "平均クリック単価":          str(cpc),
        "コンバージョン単価":        str(cpa) if cpa is not None else " --",
        # 照合用（JSON内部のみ）
...
def main():
    ...
    if not args.daily:
        print(f"\n【検索カテゴリ別（imp上位15件）】")
        print(f"  {'検索カテゴリ':<35} {'imp':>8} {'clk':>6} {'cost':>8} {'conv':>6} {'CVR':>7}")
        print("  " + "-" * 75)
