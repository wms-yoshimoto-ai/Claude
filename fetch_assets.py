...
サイトリンク / コールアウト / 構造化スニペット / 電話番号 / 画像 / 広告見出し / 説明文 の設定内容を取得する
...
【使い方】
  ...
  # 特定タイプのみ（カンマ区切り）
  python3 fetch_assets.py --site 065 --types sitelink,callout,headline,description
...
【取得対象】
  ...
  - HEADLINE（広告見出し）: テキスト（text_asset）
  - DESCRIPTION（説明文）: テキスト（text_asset）

【取得レベル】
  - campaign_asset（キャンペーンレベル）: 全タイプ
  - ad_group_asset（広告グループレベル）: HEADLINE / DESCRIPTION のみ
  - customer_asset（アカウントレベル）: SITELINK / CALLOUT / CALL
...
【注意事項】
  ...
  - IMAGE の field_type: 検索=AD_IMAGE、P-MAX=MARKETING_IMAGE/LOGO 等
"""
...
# 取得対象アセットタイプ
ASSET_TYPES_ALL = ["sitelink", "callout", "structured_snippet", "call", "image", "headline", "description"]
...
CSV_COLUMNS = [
    ...
    "asset_id",
    "asset_name",
    "level",           # CAMPAIGN / AD_GROUP / ACCOUNT
    "campaign_id",
    "campaign_name",
    "ad_group_id",
    "ad_group_name",
...
def _empty_row() -> dict:
    """全カラムを None で初期化した行を返す"""
    return {col: None for col in CSV_COLUMNS}

def _base_row(asset_type: str, asset: dict, level: str, status: str,
              campaign_id: str = "", campaign_name: str = "",
              ad_group_id: str = "", ad_group_name: str = "") -> dict:
    """共通フィールドを設定した行を返す"""
    row = _empty_row()
    row.update({
...
# ============================================================
# データ取得：キャンペーンレベル（campaign_asset）
# ============================================================
...
def fetch_sitelinks(customer_id: str, campaign_filter: str,
    ...
    for r in results:
        ...
        row = _base_row("SITELINK", asset, "CAMPAIGN", ca.get("status", ""),
                        str(cpn.get("id", "")), cpn.get("name", ""))
        row["sitelink_text"]  = sl.get("linkText", "")
        row["sitelink_desc1"] = sl.get("description1", "")
        row["sitelink_desc2"] = sl.get("description2", "")
        row["sitelink_url"]   = final_urls[0] if final_urls else ""
        rows.append(row)
    return rows
...
def fetch_callouts(customer_id: str, campaign_filter: str,
    ...
    for r in results:
        ...
        cpn   = r.get("campaign", {})
        ca    = r.get("campaignAsset", {})
        row = _base_row("CALLOUT", asset, "CAMPAIGN", ca.get("status", ""),
                        str(cpn.get("id", "")), cpn.get("name", ""))
        row["callout_text"] = co.get("calloutText", "")
        rows.append(row)
    return rows
...
def fetch_structured_snippets(customer_id: str, campaign_filter: str,
    ...
    for r in results:
        ...
        row = _base_row("STRUCTURED_SNIPPET", asset, "CAMPAIGN", ca.get("status", ""),
                        str(cpn.get("id", "")), cpn.get("name", ""))
        row["snippet_header"] = sn.get("header", "")
        row["snippet_values"] = json.dumps(values, ensure_ascii=False) if values else ""
        rows.append(row)
    return rows
...
def fetch_call_assets(customer_id: str, campaign_filter: str,
    ...
    for r in results:
        ...
        cpn     = r.get("campaign", {})
        ca      = r.get("campaignAsset", {})
        row = _base_row("CALL", asset, "CAMPAIGN", ca.get("status", ""),
                        str(cpn.get("id", "")), cpn.get("name", ""))
        row["call_phone"]   = ca_data.get("phoneNumber", "")
        row["call_country"] = ca_data.get("countryCode", "")
        rows.append(row)
    return rows
...
def fetch_image_assets(customer_id: str, campaign_filter: str,
    ...
    gaql = f"""
        SELECT
            ...
            campaign.id, campaign.name, campaign.status
        FROM campaign_asset
        WHERE campaign_asset.field_type IN ({field_types_str})
          AND campaign.status != 'REMOVED'
          {campaign_filter}
    ...
    for r in results:
        ...
        cpn      = r.get("campaign", {})
        ca       = r.get("campaignAsset", {})
        row = _base_row("IMAGE", asset, "CAMPAIGN", ca.get("status", ""),
                        str(cpn.get("id", "")), cpn.get("name", ""))
        row["image_field_type"] = ca.get("fieldType", "")
        row["image_url"]        = full_sz.get("url", "")
        row["image_width"]      = full_sz.get("widthPixels")
        row["image_height"]     = full_sz.get("heightPixels")
        row["image_mime_type"]  = img.get("mimeType", "")
        rows.append(row)
    return rows


def fetch_headlines(customer_id: str, campaign_filter: str,
                    creds: dict, token: str) -> list:
    """広告見出し（HEADLINE）をキャンペーンレベルで取得"""
    gaql = f"""
        SELECT
            asset.id, asset.name,
            ...
            campaign.id, campaign.name, campaign.status
        FROM campaign_asset
        WHERE campaign_asset.field_type = 'HEADLINE'
          AND campaign.status != 'REMOVED'
          {campaign_filter}
        ORDER BY campaign.name, asset.text_asset.text
    """
    results = gaql_request(customer_id, gaql, creds, token)
    rows = []
    for r in results:
        asset = r.get("asset", {})
        txt   = asset.get("textAsset", {})
        cpn   = r.get("campaign", {})
        ca    = r.get("campaignAsset", {})
        row = _base_row("HEADLINE", asset, "CAMPAIGN", ca.get("status", ""),
                        str(cpn.get("id", "")), cpn.get("name", ""))
        row["headline_text"] = txt.get("text", "")
        rows.append(row)
    return rows


def fetch_descriptions(customer_id: str, campaign_filter: str,
                       creds: dict, token: str) -> list:
    """説明文（DESCRIPTION）をキャンペーンレベルで取得"""
    gaql = f"""
        SELECT
            asset.id, asset.name,
            asset.text_asset.text,
            ...
            campaign.id, campaign.name, campaign.status
        FROM campaign_asset
        WHERE campaign_asset.field_type = 'DESCRIPTION'
          AND campaign.status != 'REMOVED'
          {campaign_filter}
        ORDER BY campaign.name, asset.text_asset.text
    """
    ...
    for r in results:
        ...
        cpn   = r.get("campaign", {})
        ca    = r.get("campaignAsset", {})
        row = _base_row("DESCRIPTION", asset, "CAMPAIGN", ca.get("status", ""),
                        str(cpn.get("id", "")), cpn.get("name", ""))
        row["description_text"] = txt.get("text", "")
        rows.append(row)
    return rows


# ============================================================
# データ取得：広告グループレベル（ad_group_asset）
# ============================================================

def fetch_adgroup_headlines(customer_id: str, campaign_filter: str,
                            creds: dict, token: str) -> list:
    """広告見出し（HEADLINE）を広告グループレベルで取得"""
    gaql = f"""
        SELECT
            asset.id, asset.name,
            asset.text_asset.text,
        ...
        FROM ad_group_asset
        WHERE ad_group_asset.field_type = 'HEADLINE'
          AND campaign.status != 'REMOVED'
          {campaign_filter}
        ORDER BY campaign.name, ad_group.name, asset.text_asset.text
    """
    results = gaql_request(customer_id, gaql, creds, token)
    rows = []
    for r in results:
        ...
        row = _base_row("HEADLINE", asset, "AD_GROUP", aga.get("status", ""),
                        str(cpn.get("id", "")), cpn.get("name", ""),
                        str(adg.get("id", "")), adg.get("name", ""))
        row["headline_text"] = txt.get("text", "")
        rows.append(row)
    return rows


def fetch_adgroup_descriptions(customer_id: str, campaign_filter: str,
                               creds: dict, token: str) -> list:
    """説明文（DESCRIPTION）を広告グループレベルで取得"""
    gaql = f"""
        SELECT
            asset.id, asset.name,
            asset.text_asset.text,
            ...
            ad_group.id, ad_group.name
        FROM ad_group_asset
        WHERE ad_group_asset.field_type = 'DESCRIPTION'
          AND campaign.status != 'REMOVED'
          {campaign_filter}
        ORDER BY campaign.name, ad_group.name, asset.text_asset.text
    """
    ...
    for r in results:
        ...
        adg   = r.get("adGroup", {})
        aga   = r.get("adGroupAsset", {})
        row = _base_row("DESCRIPTION", asset, "AD_GROUP", aga.get("status", ""),
                        str(cpn.get("id", "")), cpn.get("name", ""),
                        str(adg.get("id", "")), adg.get("name", ""))
        row["description_text"] = txt.get("text", "")
        rows.append(row)
...
def fetch_account_sitelinks(customer_id: str, creds: dict, token: str) -> list:
    ...
    for r in results:
        ...
        final_urls = asset.get("finalUrls", sl.get("finalUrls", []))
        row = _base_row("SITELINK", asset, "ACCOUNT", ca.get("status", ""))
        row["sitelink_text"]  = sl.get("linkText", "")
        row["sitelink_desc1"] = sl.get("description1", "")
        row["sitelink_desc2"] = sl.get("description2", "")
        row["sitelink_url"]   = final_urls[0] if final_urls else ""
...
def fetch_account_callouts(customer_id: str, creds: dict, token: str) -> list:
    ...
    for r in results:
        ...
        co    = asset.get("calloutAsset", {})
        ca    = r.get("customerAsset", {})
        row = _base_row("CALLOUT", asset, "ACCOUNT", ca.get("status", ""))
        row["callout_text"] = co.get("calloutText", "")
        rows.append(row)
    return rows
...
def fetch_account_call(customer_id: str, creds: dict, token: str) -> list:
    ...
    for r in results:
        ...
        ca_data = asset.get("callAsset", {})
        ca      = r.get("customerAsset", {})
        row = _base_row("CALL", asset, "ACCOUNT", ca.get("status", ""))
        row["call_phone"]   = ca_data.get("phoneNumber", "")
        row["call_country"] = ca_data.get("countryCode", "")
        rows.append(row)
    return rows
...
def save_json(rows: list, site_id: str, ts: str) -> Path:
    ...
    level_count = Counter(r["level"]      for r in rows)
...
def main():
    ...
    parser.add_argument("--types",     default="",     help="取得タイプ（カンマ区切り: sitelink,callout,structured_snippet,call,image,headline,description）")
    ...
    if "headline" in types_to_fetch:
        print("[INFO] 広告見出し（campaign_asset）を取得中...")
        rows = fetch_headlines(cid, campaign_filter, creds, token)
        print(f"  → {len(rows)} 件")
        all_rows.extend(rows)

    if "description" in types_to_fetch:
        print("[INFO] 説明文（campaign_asset）を取得中...")
        rows = fetch_descriptions(cid, campaign_filter, creds, token)
        print(f"  → {len(rows)} 件")
        all_rows.extend(rows)

    # ── 広告グループレベル取得（HEADLINE / DESCRIPTION）──
    if not campaign_id or campaign_filter:
        if "headline" in types_to_fetch:
            print("[INFO] 広告見出し（ad_group_asset）を取得中...")
            rows = fetch_adgroup_headlines(cid, campaign_filter, creds, token)
            print(f"  → {len(rows)} 件")
            all_rows.extend(rows)

        if "description" in types_to_fetch:
            print("[INFO] 説明文（ad_group_asset）を取得中...")
            rows = fetch_adgroup_descriptions(cid, campaign_filter, creds, token)
            print(f"  → {len(rows)} 件")
            all_rows.extend(rows)
    ...
    # ── 出力 ──
    if all_rows:
        json_path = save_json(all_rows, args.site, ts)
        save_csv(all_rows, args.site, ts)
        # タイムスタンプをstdoutに出力（run_fetcher.sh が拾えるように）
        print(f"[OUTPUT_TS] {ts}")
    else:
